#Creates pre- and post branches of benchmarks for Nullgtn
#!/bin/bash
...
tail -n +2 "$CSV_FILE" | while IFS=, read -r ProjectName Link pre_nullgtn_without_s post_nullgtn_without_s _; do
  # Trim any leading or trailing whitespace from the variables
  ProjectName=$(echo $ProjectName | xargs)
  Link=$(echo $Link | xargs)
  pre_nullgtn_without_s=$(echo $pre_nullgtn_without_s | xargs | sed 's/,$//')
  post_nullgtn_without_s=$(echo $post_nullgtn_without_s | xargs | sed 's/,$//')
  ...
  echo "pre_nullgtn_without_s commit hash: $pre_nullgtn_without_s"
  echo "post_nullgtn_without_s commit hash: $post_nullgtn_without_s"
  echo "----------------------------------------"
  ...
  for column in "pre_nullgtn_without_s" "post_nullgtn_without_s"; do
